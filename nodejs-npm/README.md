...
|---------|---------|------|---------------|
| `lodash` | 4.17.15 | 3 | **Partial remediation path**: 4.17.15 (3 CVEs) → 4.17.19 (2 CVEs) → 4.17.21 (0 CVEs) |
| `tar` | 6.1.0 | 6 | **Multiple vulnerabilities**: High CVE count, tests bulk vulnerability listing & pagination |
| `axios` | 0.21.1 | 1 | **SSRF vulnerability**: Tests single critical CVE remediation |
| `minimist` | 1.2.5 | 1 | **Prototype pollution**: Common transitive dependency vulnerability |
| `jsonwebtoken` | 8.5.1 | 1 | **Auth library CVE**: Tests security-sensitive package handling |
| `moment` | 2.29.1 | 1 | **Deprecated package**: Tests handling of unmaintained packages with CVEs |
| `express` | 4.17.1 | 2 | **Framework CVEs**: Tests vulnerabilities in core framework |
| `uuid` | ^9.0.0 | 0 | **Clean baseline**: No known vulnerabilities, tests clean component handling |
| `@angular/core` | 14.0.0 | 0 | **Scoped package PURL**: Tests `pkg:npm/%40angular/core@14.0.0` URL encoding |
| `@babel/core` | 7.12.0 | 0 | **Scoped package PURL**: Tests `pkg:npm/%40babel/core@7.12.0` URL encoding |
...
| `request` | 2.88.2 | 0 | **Deprecated/Unmaintained**: Tests UNMAINTAINED_NON_VULNERABLE validation |
| `core-js` | 2.6.12 | 0 | **Outdated version**: Tests OUTDATED_NON_VULNERABLE validation (latest is 3.x) |
| `chokidar` | 3.5.3 | 0 | **Near-latest version**: Tests ALREADY_LATEST_VERSION validation |
...
|-----------------|---------|----------|
| **CAN_REMEDIATE** | `lodash@4.17.15` | Has CVEs, not latest, can upgrade to 4.17.21 |
| **ALREADY_LATEST_VERSION** | `chokidar@3.5.3` | Near-latest version, limited upgrade path |
| **OUTDATED_VULNERABLE** | `tar@6.1.0` | Outdated with vulnerabilities, can remediate with warning |
| **OUTDATED_NON_VULNERABLE** | `core-js@2.6.12` | Outdated (2.x vs 3.x), no CVEs, info message |
| **UNMAINTAINED_VULNERABLE** | `moment@2.29.1` | Deprecated with CVE, cannot remediate |
| **UNMAINTAINED_NON_VULNERABLE** | `request@2.88.2` | Deprecated, no CVEs, warning only |
| **EOL_VULNERABLE** | - | Needs EOL package with CVEs (ecosystem-specific) |
| **EOL_NON_VULNERABLE** | - | Needs EOL package without CVEs (ecosystem-specific) |
